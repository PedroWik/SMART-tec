...
<body>
    ...
    #    $id_dest = $ver_sm['id_user_dest'];
        ...
        $amigos = mysqli_query($link, "SELECT * from `amigo` WHERE id_user='$id_user'");
        while ($amigo = mysqli_fetch_assoc($amigos)) {
            $id = $amigo['id_dest'];

            $u->conectar("glou","localhost","root","");         
            if ($u->contar_sms_nao_lido($id,$id_user)){
            }
            ...
            if (isset($_POST['id_dest'])) {
                $id_destino = addslashes($_POST['id_dest']);
